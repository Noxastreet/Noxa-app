...
- Consequence: several files PR #135 added were not originally present on `main`. A follow-up correction pass has since completed the PR #135 contract reconciliation this audit's original scope required but missed — see `docs/audit/PR135_CONTRACT_RECONCILIATION.md` for the full still-valid/superseded/stale/needs-decision classification. `docs/GROUP_DRIVE.md` was imported as canonical, still-valid architecture as a result; `NoxaFloatingCard`/`IdentityOrb` remain not-yet-present as code (their design intent is recorded, not imported); the MVP-process docs (`docs/mvp/*`, `docs/MVP_COMPLETION_MASTER.md`, etc.) were reviewed and mostly classified as superseded by `main`'s own `docs/ROADMAP.md`/`docs/ai-design-library/*`, so they remain absent from `main` deliberately, not by oversight. PR #135 itself is currently `draft`, `mergeable_state: clean` against its recorded base, but its branch diverged from `main` 8 commits after a point that is now 30 commits behind `main`'s tip (onboarding, profile, garage, notifications, settings, auth and map-follow work all landed on `main` after PR #135 branched) — reconciling PR #135's *branch* itself (merging or closing it) is still separate, unstarted work, not part of this audit.
- Existing unrelated P0 (carried forward, not touched here): **widening an already-active personal Live Drive audience must require explicit re-confirmation.** Current code widens silently (`app/(tabs)/index.tsx` visibility-mode change path). This is a privacy behavior fix, not a visual-foundation change, and must ship in its own commit/PR — see `docs/VISUAL_ARCHITECTURE_V2.md` §16. **This P0 has priority over broad Visual Architecture V2 screen rollout** — it should land before, or at minimum not be blocked behind, Stage 1 (Onboarding) and later stages.
- Stage naming (per `docs/CURRENT_STATE.md`): this reconciliation audit and `docs/VISUAL_ARCHITECTURE_V2.md` together are **Stage 0A — Visual V2 canonical reconciliation — COMPLETE / APPROVED**. The additive-tokens-only commit recommended at the end of this document is **Stage 0B — additive Visual Foundation V2 tokens — NEXT**. Then Stage 1 Onboarding, Stage 2 Home/Map, Stage 3 Event Detail, Stage 4 Garage/Vehicle, and Stage 5 Group Drive / Active Drive (MVP-required, but starting only after explicit implementation authorization and prerequisite backend/security planning) — followed by the static MVP release candidate and physical Android/iOS validation.
...
**Target rule (corrected):** 24px is the *preferred* phone gutter, not a rigid constant — it may adapt down within a 16–24px range on narrow screens for readability/accessibility/touch-target/layout-stability reasons (`docs/VISUAL_ARCHITECTURE_V2.md` §4).

**Comparison:** Two different current gutter values depending on code path: `NoxaScreen`'s own `padded` style uses `spacing.lg` (20px, flat, non-responsive; `src/components/ui/NoxaScreen.tsx:26-30`), while `Screen`/`useResponsive`'s adaptive gutter is `clamp(16, width*0.045, 24)` on phones, 32 on tablets (`src/hooks/useResponsive.ts:50-52`, consumed by `src/components/layout/Screen.tsx:51-53`). **`useResponsive`'s clamp already implements the corrected target rule almost exactly** — it's a responsive 16–24px range with 24 as the ceiling, not a mismatch. `NoxaScreen`'s flat 20px default is the actual gap: it doesn't reach the 24 ceiling on wider phones, and it isn't responsive at all on narrow ones.
...
**Recommended change:** No new rigid token is needed — `useResponsive`'s existing `clamp(16, width*0.045, 24)` already matches the corrected rule. The gap is narrower than originally framed: decide, screen by screen as each of the five reference experiences is implemented, whether it adopts `useResponsive`'s adaptive gutter directly. **Do not change `NoxaScreen`'s global default padding as part of this decision** — that's a shared primitive touching every existing screen, not just the five V2 references, and is explicitly out of scope for now.

**Risk:** Low — this is now a per-screen adoption decision (which gutter source a given V2 screen reads from), not a shared-primitive default change.
...
- **Corrected target rule:** shadow is allowed on exactly **one** surface in the hierarchy — the L2 sheet (one restrained shadow/elevation). L1 content is flat (no shadow at all). L3 destructive content **replaces L2's content inside the same shell** — it does not add a second, stacked elevated surface or a second shadow of its own (`docs/VISUAL_ARCHITECTURE_V2.md` §2). Current `shadows.card` is applied broadly and unconditionally (`NoxaCard`, `NoxaSheet`, garage vehicle cards, empty states, `CanonicalEventDetailScreen.tsx`'s sibling info/map/going/organizer/details cards) with no L0–L2 distinction encoded anywhere — every one of those is L1 content that should be flat, not L2.
- **The "cardboard" feeling does not come from `NoxaCard`/`NoxaSheet` misuse — it comes from those primitives being dead code.** Grep across the whole repo found zero `<NoxaCard` and zero `<NoxaSheet` instantiations in `app/` or `src/`. Every real screen (`(tabs)/index.tsx`, `CanonicalEventDetailScreen.tsx`, `vehicle-details.tsx`, `garage.tsx`) reimplements its own bordered `View` (`infoCard`, `organizerCard`, `vehicleCard`, `stateCard`, …) with `borderWidth:1 + colors.surface + shadows.card` repeated ad hoc. `CanonicalEventDetailScreen.tsx` alone has five independent sibling "cards" (`infoCard`, `mapCard`, `goingCard`, `organizerCard`, `detailsCard`) — not nested, but each one is its own bordered, shadowed container, which is exactly the "wall of cards" the design brief is solving for, and exactly the pattern that should become flat L1 content instead.

**Recommended change:** Add the missing 3 grey tokens; add an explicit `radius.sheet` (28, top-only via component styling, not a single scalar); introduce a documented convention (comment/lint note, not a new system) that `shadows.*` is reserved for the L2 sheet shell only — L1 content never carries it, and L3 inherits L2's existing shadow rather than adding its own. The larger structural fix — retiring the five independent ad hoc "cards" on Event Detail/Garage/Vehicle in favor of flat L1 content with hairline separation — is the real Stage 1+ work, not a token change.
...
**Update from the PR #135 contract reconciliation** (see `docs/audit/PR135_CONTRACT_RECONCILIATION.md`): `docs/GROUP_DRIVE.md` — the full Group Drive data model, state machines, privacy matrix, RLS requirements and MVP scope — has since been imported into `main` as canonical, still-valid architecture (it was documentation-only on PR #135 and nothing on `main` contradicts it). It resolves the Leave-vs-End question this section originally left open: **MVP host action is "End Drive" only — the host cannot Leave**, a non-host participant's action is "Leave Drive" only. `docs/VISUAL_ARCHITECTURE_V2.md` §13 has been corrected to match. The Product Owner has since resolved the remaining questions: **Group Drive is MVP-required** (the final major MVP system, Stage 5, beginning only on explicit authorization), and naming is final — **Group Drive** = feature/entity, **Active Drive** = fullscreen runtime, **Live** = status label, **Live Drive** = the existing personal sharing feature only. Group Drive is not Crew Convoy, which stays frozen V2.

**Recommended change:** None in Stage 0/0B. The target's Active Drive contract (§10.5 / §13 of `VISUAL_ARCHITECTURE_V2.md`, backed by `docs/GROUP_DRIVE.md`) is recorded as the eventual visual target for whenever Group Drive backend work is scheduled (per `docs/ai-design-library/04-mvp-v2-boundary.md`, Convoy/complex social mechanics remain frozen until MVP runtime passes). This audit explicitly does **not** propose building Group Drive, does not touch `convoy-setup.tsx`, and does not touch `liveDrive.ts`.
...
## Stage 0B recommended first implementation commit (additive Visual Foundation V2 tokens)
