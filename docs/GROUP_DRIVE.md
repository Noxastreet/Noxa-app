...
This document is the canonical specification for the Group Drive feature. Phase 1 now has a review-only draft migration and local verification harness. Phase 2 has a separate stacked review-only implementation draft for create/invitation screens and the independent `drive-route` Edge Function. Neither draft has been applied or deployed to production, and Phase 2 has no Home/Map entry or live-location runtime. Implementation proceeds only through the phased plan defined below, each phase as its own scoped, reviewed change. **Being MVP-required does not authorize any production Supabase change** — Group Drive's schema, RLS, RPCs and Edge Function each still require their own scoped review and the existing production approval gates (§8, §11, §13).

**Temporary sequencing exception — 2026-08-19:** the Product Owner explicitly authorized free Phase 2 preparation while the Phase 1 hosted Preview gate remains unavailable on the current Supabase plan. Phase 2 is therefore stacked on the Phase 1 draft and remains non-mergeable to `main` until Phase 1's required hosted verification and production approval gates are satisfied. This exception authorizes review-only client/function files, not a deployment, production migration, Home/Map integration, or live-location implementation.
...
The fields below are the canonical contract implemented by the Phase 1 review
draft. That draft remains unapplied to production.
...
Because Crew context is optional provenance rather than ownership, deleting a
Crew clears `drive_sessions.crew_id` and every matching invitation's
`source_crew_id` even when the drive is already terminal. Those referential
cleanups are the only allowed post-terminal field changes; they must not block
Crew or account deletion.
...
| Field | Notes |
|---|---|
| `id` | opaque primary key used only as the privacy-safe Realtime deletion key |
| `drive_session_id`, `user_id` | required foreign-key pair with a unique constraint — one current row per participant per drive |
| `latitude` / `longitude` | required |
| `heading` | nullable |
...
- **`SELECT` requires both conditions simultaneously: `drive_sessions.status = 'active'` *and* the caller's own `drive_participants.status = 'active'` for that `drive_session_id`.** `accepted` status alone — before the session actually starts — grants **no** location access: a participant cannot write their location, cannot read anyone else's, and cannot subscribe to current-row changes while only `accepted`.
- **Clients receive no direct `INSERT` or `UPDATE` table privilege.** Every location write uses the `noxa_upsert_drive_location` `security definer` RPC, which derives `user_id` from `auth.uid()`, rechecks both active statuses and blocking, and acquires the session row followed by the participant row with `FOR SHARE` before touching the location row. This lock order serializes the write against Leave, Remove, End, Cancel, and Expire so a late concurrent upsert cannot survive their synchronous cleanup.
- Supabase Realtime Postgres Changes applies table RLS to readable current-row changes, but its [`DELETE` events are not filterable and cannot be checked against the deleted row's RLS](https://supabase.com/docs/guides/realtime/postgres-changes#receiving-old-records). Therefore the Realtime-visible primary key is the unrelated opaque `id`; `(drive_session_id, user_id)` remains unique but is **not** the primary key. A deletion packet can reveal only a random location-state ID, never the drive or participant identifier. Phase 3 must still prove hosted Realtime delivery and deletion behavior before production.
...
- `drive_location_state`: `select` — requires, simultaneously, `drive_sessions.status = 'active'` for the row's session and the caller's own `drive_participants.status = 'active'` for that same session (§4.5); direct `insert`/`update`/`delete` table privileges — none. `noxa_upsert_drive_location` is the only client write path and must derive the writer from `auth.uid()`, repeat the active/block checks, and lock session then participant before upserting. Deletion remains automatic through state-transition RPCs (§4.5, §5.4). The opaque primary key prevents Realtime's unfilterable, non-RLS `DELETE` packets from exposing `drive_session_id` or `user_id`; hosted Realtime behavior remains an explicit Phase 3 gate.
...
- **Phase 2 — Create/Invitation flows.** Drive Details, Route Builder, Add Participants, Privacy and Schedule, Route Review, My Group Drives, Invitation Detail, Drive Details screens. `drive-route` Edge Function. No live-location code yet. **Current status:** implemented in a separate stacked review draft; static verification exists, but the function is undeployed, no Group Drive backend is present in production, and native multi-account runtime is not yet verified. See `docs/security/NOXA_GROUP_DRIVE_PHASE_2_RUNBOOK.md`.
