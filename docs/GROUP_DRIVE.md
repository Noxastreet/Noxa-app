...
- `active → completed`: host explicit "End Drive"; `end_reason = 'host_completed'`. Not automatic on reaching the last stop in MVP (see §9).
...
No row transitions `accepted → active` individually or lazily — it happens once, for every currently-`accepted` row, at the single moment the session starts. A user who was never `accepted` before that moment cannot join afterward in MVP (§4.3).
