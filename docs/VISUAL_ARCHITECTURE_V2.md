...
### 10.5 Active Drive
Naming is canonical and final (`docs/GROUP_DRIVE.md` §1): **Group Drive** is the feature/entity, **Active Drive** is this fullscreen runtime screen/state, **Live** is a status label only, and **Live Drive** refers exclusively to the separate existing personal location-sharing feature. Never label this runtime "Live Drive."
...
- Group Drive / Active Drive application code — it does not exist yet. Group Drive **is MVP-required** (Product Owner decision) and is scheduled as **Stage 5, the final major MVP system**, beginning only after explicit implementation authorization and prerequisite backend/security planning. `docs/GROUP_DRIVE.md` is the target architecture; this document specifies its target visual contract. Neither document authorizes starting implementation, and neither authorizes any production Supabase change — Group Drive's schema, RLS, RPCs and Edge Function each require their own scoped review and the existing production gates. Group Drive is **not** the crew-scoped "Crew Convoy" flow in `app/convoy-setup.tsx`, which remains frozen V2 and is not reused as the Group Drive domain.
